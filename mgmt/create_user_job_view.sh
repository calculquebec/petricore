...
commands="CREATE USER '${newUser}'@'${loginhost}' IDENTIFIED BY '${newDbPassword}';USE slurm_acct_db; CREATE VIEW user_job_view AS SELECT id_user, id_job, job_name FROM ${domain}_job_table; GRANT SELECT ON user_job_view TO '${newUser}'@'${loginhost}';"
